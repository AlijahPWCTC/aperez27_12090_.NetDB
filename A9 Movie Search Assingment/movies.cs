...
namespace A9_Movie_Search_Assingment
{
    public class movies : media
    {
        ...
        public void display()
        {
            ...
            Console.WriteLine($"ID: {this.ID} Title: {this.title} Genres: {holder}");
        }
        ...
        public static void addMovie(List<movies> movieList){
            ...
            int newIds = (movieList[movieList.Count-1].ID+1);
        ...
        public static void addMovieJson(List<movies> movieList){
            String newTitle = "";
            try{
                Console.WriteLine("Enter Movie Title and Year: ");
                newTitle = Console.ReadLine();
            ...
            movies tempMovie = new movies(newIds, newTitle, genreArray);
            movieList.Add(tempMovie);
            var lines = File.ReadAllLines("movies.json");
            var movieJson = JsonNet.Serialize(tempMovie);
            lines[lines.Length-3] +=",";
            lines[lines.Length-2] = $"{movieJson}\n";
            File.WriteAllLines("movies.json", lines);
        }

        public static void jsonMovieOptions(List<movies> movieList){
            int choice=-1;
            while (choice!=3){
                try{
                    Console.WriteLine("Type 1 for Display options Type 2 to add a new entry Type 3 to exit: ");
