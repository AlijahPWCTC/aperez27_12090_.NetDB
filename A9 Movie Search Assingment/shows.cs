...
namespace A9_Movie_Search_Assingment
{
    public class shows: media
    {
        ...
        public void display()
        {
            ...
            Console.WriteLine($"ID: {this.ID} Title: {this.title} Season: {this.season} Episode: {this.episode} Writers: {holder}");
        }
        ...
        public static void addShow(List<shows> showList){
            ...
            int newIds = (showList[showList.Count-1].ID+1);
        ...
        public static void addShowsJson(List<shows> showList){
            String newTitle = "";
            try{
                Console.WriteLine("Enter Show Title: ");
                newTitle = Console.ReadLine();
            ...
            showList.Add(tempShow);
            String newEntry = $"{newIds},{newTitle},{newSeason},{newEpisode},{writerFile}";
            var lines = File.ReadAllLines("shows.json");
            var showJson = JsonNet.Serialize(tempShow);
            lines[lines.Length-3] +=",";
            lines[lines.Length-2] = $"{showJson}\n";
            File.WriteAllLines("shows.json", lines);
        }

        public static void jsonShowsOptions(List<shows> showList){
            int choice=-1;
            while (choice!=3){
                try{
                    Console.WriteLine("Type 1 for Display options Type 2 to add a new entry Type 3 to exit: ");
